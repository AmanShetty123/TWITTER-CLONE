...
export const getUserProfile = async (req, res) => {
  ...
  try {
    ...
    console.log("Error in UserProfileController", error.message);
  ...
  try {
    ...
    if (currentPassword && newPassword) {
      ...
      if (newPassword.length < 6) {
        return res
          .status(400)
          .json({ error: "The password must be atleast 6 characters long  " });
      }
    ...
    //password should be null in response and not in database
    user.password = null;
